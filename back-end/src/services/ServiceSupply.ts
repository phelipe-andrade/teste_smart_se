...
class ServiceSupply {
  ...
  async getSupply(id: number): Promise<Supply> {
    ...
    if(!supplyExists) throw new AppError(`Não existe nenhum abastecimento cadastrado com o id de n°: ${id}`);
    return supplyExists;
  }

  async create(supply: CreateSupplyDTO, id: number): Promise<ResultResponseMessage> {
  ...
  async update(supply: UpdateSupplyDTO): Promise<ResultResponseMessage> {
    const {id} = supply;

    const supplyExists = await prisma.supply.findFirst({ where: {id}});      
    if(!supplyExists) throw new AppError(`Abastecimento com id de n°: ${id} não encontrado.`);
